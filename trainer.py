...
class Trainer:
    def __init__(self, model, CFG=None, loss=None, train_loader=None, val_loader=None):
        ...
        if self.CFG.amp is True:
            self.scaler = torch.cuda.amp.GradScaler(enabled=True)       # 自动混合精度，可以在神经网络推理过程中，针对不同的层，采用不同的数据精度进行计算，从而实现节省显存和加快速度的目的。
        self.loss = loss
        self.model = nn.DataParallel(model.cuda())
        self.train_loader = train_loader
        self.val_loader = val_loader
        self.optimizer = get_instance(
            torch.optim, "optimizer", CFG, self.model.parameters())     # 获取属性adam的值
    ...
    def _save_checkpoint(self, epoch):
        ...
        state = {
            ...
            'epoch': epoch,
            'state_dict': model.state_dict(),       # self.
